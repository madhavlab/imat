...
## How do you integrate different ML models?
...
2. Single Model Approach - A single model that predicts melody estimates and corresponding confidence. 
...
def get_melody_json(model, X):
 """Extract melody from spectrogram using the unified model."""
...
def conf_values(model, X):
 """Get confidence values from the unified model."""
 X = np.expand_dims(X, axis=0)
 X = (X - mean) / std
 X = X[:, :, :, np.newaxis]
