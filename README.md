...
``` This tool is only tested on Linux, and not on Mac or Windows. ```
...
- **STEP 1:** Click on the 'Upload audio' button and browse through your system to annotate the audio you wish. Please make sure you upload a '.wav' audio file. The maximum duration allowed for the audio is 30 seconds. You can listen to the audio by clicking play/pause button.
...
1. Ideally, the estimated melody should align or overlay with the F0 contour of the spectrogram. The user visually observes the region where the estimated melody is incorrect and selects a particular range of the spectrogram to annotate the incorrect melody. Once selected, the user clicks on `Annotate'.
...
9. The points from 1 to 7 are repeated for *s* iterations of Adaptive Annotation until the user observes a very few incorrect melody points are present.
