...
audio_files = sorted(glob('../../../Datasets/adc2004/Wavfile'+'*.wav'))
pitch_files = sorted(glob('../../../Datasets/adc2004/Pitchlabel'+'*.csv'))

# print(len(audio_files),len(pitch_files))
...
# meta_trained_classifier.load_weights('../weights/meta_model/pre/meta_weights-500')  #load the pre-train weights
...
# meta_trained_conf.load_weights('../weights/meta_model/conf/meta_weights-500')
...
# alpha = 2.e-5  # 5.e-5  --> only 3 times retraining
...
def conf_loss(yt, yp, yc, ts):
    ...
    print(f'shape of yp..{tf.shape(yp[0])} {tf.shape(indexing)}')
...
def plot_melody(fe_model,S):
    ...
    Input: model, wavfile 5 sec chunk
...
def verify_updation(yp, yt):
    true_indx = tf.argmax(yt, axis=-1)
    pred_indx = tf.argmax(yp, axis=-1)
    print(true_indx, pred_indx)


def verify_user_changes(yp, yt, indx):
    print('Support index:', indx)

    for i in range(len(yt)):
        print(i, yp[i], yt[i])


def verify_onehot(yp, yt):
    true_indx = tf.argmax(yt, axis=-1)
    pred_indx = tf.argmax(yp, axis=-1)
    # print(pred_indx,true_indx)
...
def aml_test(S, active_frames, gfv,fileid):
    ...
    if (fileid==0):
        ...
        # for i in range(len(fe_model.layers)):
        #     print(fe_model.layers[i].trainable)
        meta_pre_weights = fe_model.get_weights()
        meta_conf_weights = conf_model.get_weights()
